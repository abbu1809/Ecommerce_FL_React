...
import { formatDateTime as _formatDateTime } from "../../utils/dateUtils";
...
const getLatestStatus = (order) => {
  if (
    ...
    // Sort by timestamp descending and get the first (latest) status
    const sortedHistory = [...order.tracking_info.status_history].sort(
      (a, b) => new Date(b.timestamp) - new Date(a.timestamp)
    );
    return sortedHistory[0].status;
  }
  // Fallback to order status if no tracking history
  return order.status;
};
